...
namespace BankingAppDataTier.Providers
{
    public class DatabasePlasticsProvider : IDatabasePlasticsProvider
    {
        ...
        public DatabasePlasticsProvider(IConfiguration configuration, IMapperProvider _mapperProvider)
        {
        ...
        public bool DeleteAll()
        {
            using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
            {
                ...
                try
                {
                    command.CommandText = $"DELETE FROM {PlasticsTable.TABLE_NAME} WHERE 1=1";
